    ...
    <div>
      ...
      <div className="fixed flex justify-between items-center z-50 bg-transparent top-0 left-0 right-0 font-[first] lg:justify-between backdrop-blur-sm py-5 px-20">
        ...
        <div className="relative h-6 w-10 cursor-pointer" onClick={toggleMenu}>
          ...
          ></div>
          <div
            className={`absolute top-1.2 left-0 h-[2px] w-10 ${darkMode ? "bg-[#F1F3DF]":"bg-black"}  transition-all ${
        ...
        <div
          ref={menuRef}
          className={`absolute top-2 -z-10 right-20 ${darkMode ? "bg-[#F1F3DF]":"bg-black"} shadow-lg rounded-lg p-10 space-y-3 text-left ${
            isOpen ? "block" : "hidden"
          }`}
        >
          <div
          ...
          >
            ...
            >
              Home
              <span className={`block ${darkMode ? "bg-black":"bg-[#F1F3DF]"} rounded-sm h-[2px] w-1 mx-auto mt-1 transition-all duration-300 group-hover:w-full`}></span>
            </button>
          </div>
          <div
